...
<div class='viewHeader'>
	<a class='addLink' href='<?= site_url() . 'admin/' . CURRENT_CONTROLLER . '/add' ?>'>Add new category</a>
	<div class='viewOpts'>
	<form id="orderByForm" name="orderByForm" action="<?= site_url() . 'admin/' . CURRENT_CONTROLLER ?>" method="POST">
		...
		<select id='o' name='o'>
			...
			<option<?= ($o == 'name ASC') ? ' SELECTED' : ''; ?> value='name ASC'>Name (A-Z)</option>
			<option<?= ($o == 'name DESC') ? ' SELECTED' : ''; ?> value='name DESC'>Name (Z-A)</option>
		</select>
...
<tr class="<?= (($idx%2) ? 'even' : 'odd'); ?>">
	...
	<td><a href="<?= site_url() . 'admin/' . CURRENT_CONTROLLER . '/edit/' . $item['id']; ?>" title="Edit Category"><img src='<?= site_url(); ?>images/icons/folder_edit.png' alt='Edit' /></a></td>
...
<? if ($total_pages > 1): ?>
	...
	<? endforeach; ?>
<? else: ?>
	<span>Displaying <strong><?= $total_items ?></strong> categor<?= ($total_items <> 1) ? 'ies' : 'y'; ?></span>
<? endif; ?>
&nbsp;
</div>
<div class='viewBtns'>
...
<?= form_close(); ?>
<? else: ?>
<p style="color: #FF0000;">There are no categories in the system</p>
<p>This is bad since you need them for the code snippets.</p>
<p><a href="<?= site_url(); ?>admin/<?= CURRENT_CONTROLLER ?>/add"><img src="<?= site_url(); ?>images/icons/folder_add.png" class="lico" />Add atleast one category</a></p>
<? endif; ?>
<br class='clear' />
<? tpl_footer(); ?>