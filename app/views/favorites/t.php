<? tpl_header(); ?>
<h1>Latest favorite code tagged <em><?= $tag ?></em></h1>
<? if (!empty($items)): ?>
...
<? else: ?>
	<? /* page link */ ?>
	<a class='ajaxLink' href="<?= site_url(); ?>favorites/ts/<?= stubify($tag) . '/' . $p ?>"><?= $p ?></a>&nbsp;
<? endif; ?>
<? endforeach; ?>
</div>
<? endif; ?>
<? else: ?>
<p class='error'>No favorite code snippets to display that match <strong><?= $searchTerm ?></strong></p>
<? endif; ?>
<? tpl_footer(); ?>