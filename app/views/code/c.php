<? tpl_header(); ?>
<h1>Latest code in <em><?= $category['name'] ?></em></h1>
<? if (!empty($items)): ?>
...
<? else: ?>
	<? /* page link */ ?>
	<a class='ajaxLink' href="<?= site_url(); ?>code/c/<?= urlencode(strtolower($category['name'])) . '/' . $p ?>"><?= $p ?></a>&nbsp;
<? endif; ?>
<? endforeach; ?>
</div>
<? endif; ?>
<? else: ?>
<p class='error'>No code snippets to display</p>
<? endif; ?>
<? tpl_footer(); ?>